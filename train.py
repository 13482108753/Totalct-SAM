...
parser.add_argument('--work_dir', type=str, default='/content/drive/MyDrive/checkpointsss/')
...
class BaseTrainer:
    ...
    def set_optimizer(self):
        ...
        self.optimizer = torch.optim.AdamW([
            {'params': sam_model.image_encoder.parameters()}, # , 'lr': self.args.lr * 0.1, 'requires_grad': False},
        ...
        ], lr=self.args.lr, betas=(0.9,0.9999), weight_decay=self.args.weight_decay)
    ...
    def train_epoch(self, epoch, num_clicks):
        ...
        for step, (image3D, gt3D) in enumerate(tbar):
            ...
            with my_context():
                ...
                image3D = self.norm_transform(image3D.squeeze(dim=1)) # (N, C, W, H, D) .squeeze(dim=1)
                ...
                # 保存和可视化分割结果
                if not self.args.multi_gpu or (self.args.multi_gpu and self.args.rank == 0):
                    if epoch % 10 == 0:  # 保存每10个epoch的结果
                       save_dir = join(MODEL_SAVE_PATH, f'results_epoch_{epoch}')
                       os.makedirs(save_dir, exist_ok=True)

                       with torch.no_grad():
                # 获取预测结果
                          pred_masks, _ = self.interaction(sam_model, image_embedding, gt3D, num_clicks=11)

                # 保存预测结果
                          for idx, (pred_mask, gt_mask) in enumerate(zip(pred_masks, gt3D)):
                             save_path = join(save_dir, f'result_{idx}.png')
                             visualize_segmentation_results(pred_mask, gt_mask, save_path)
    ...
    def visualize_segmentation_results(pred_mask, gt_mask, save_path):
        transform = transforms.Compose([transforms.ToPILImage(), transforms.Resize((256, 256))])

    # 将tensor转为PIL图像
        pred_image = transform(pred_mask.cpu())
        gt_image = transform(gt_mask.cpu())

    # 创建一个图像，包含预测和真实掩码
        fig, axes = plt.subplots(1, 2, figsize=(8, 4))
        axes[0].imshow(np.asarray(pred_image), cmap='gray')
        axes[0].set_title('Predicted Mask')

        axes[1].imshow(np.asarray(gt_image), cmap='gray')
...
def main():
    ...
    if args.multi_gpu:
        ...
        input_tensor = torch.randn((1,) + (128,128,128))  # Adjust input size accordingly
        flops, params = calculate_flops_and_params(model, input_tensor)
        print(f"Model FLOPs: {flops / 1e9} GFLOPs")
        print(f"Model parameters: {params / 1e6} million")
        # Create trainer
