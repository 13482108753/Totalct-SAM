...
class Dataset_Union_ALL(Dataset): 
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            sitk_image = sitk.ReadImage(self.image_paths[0])
        ...
        if subject.label.data.sum() <= self.threshold:
            '''files = os.listdir("/content/drive/MyDrive/SAM-Med3D-main/segment_anything/data/train/adrenal_gland_left/ct/imagesTr/")
            idx_list = [int(x[1:5]) for x in files]
            idx = idx_list[np.random.randint(len(idx_list))]
    ...
    def _set_file_paths(self, paths):
        ...
        for path in paths:
            ...
            if os.path.exists(d):
                for name in os.listdir(d):
                    base = os.path.basename(name).split('.nii.gz')[0]
                    image_path = os.path.join(path, f'images{self.data_type}', f'{base}.nii.gz')
                    self.label_paths.append(image_path.replace('images', 'labels'))
